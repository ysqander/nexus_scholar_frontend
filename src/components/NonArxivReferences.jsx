...
function NonArxivReferences({ paper, showNonArxivRefs, setShowNonArxivRefs }) {
  ...
  const nonArxivRefs = paper.references.filter(
    (ref) => !ref.is_available_on_arxiv
  )
  ...
  return (
    <div className="mb-6">
      ...
      >
        <span>
          Non-ArXiv References found{' '}
          <span className="text-sm text-gray-500">(not in context)</span>
        </span>
        {showNonArxivRefs ? (
          ...
          <ul className="space-y-2">
            {nonArxivRefs.map((ref, index) => (
              <React.Fragment key={index}>
                <li className="retro-text-light">{ref.text}</li>
                <hr className="my-2" />
              </React.Fragment>
            ))}
          </ul>
