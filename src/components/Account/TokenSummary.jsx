    ...
    return Object.entries(cacheUsage.chat_history).map(([month, tiers]) => (
      <React.Fragment key={month}>
        <tr className="bg-gray-200">
          <td colSpan="6" className="font-bold py-2 px-4">{month}</td>
        </tr>
        {Object.entries(tiers).flatMap(([tier, chats]) =>
          chats.map((chat, index) => (
            <tr key={`${month}-${tier}-${index}`} className="border-b">
              ...
              <td className="py-2 px-4">{tier === 'pro' ? chat.token_hours_used.toFixed(4) : '-'}</td>
              <td className="py-2 px-4">{tier === 'base' ? chat.token_hours_used.toFixed(4) : '-'}</td>
            </tr>
          ))
        )}
